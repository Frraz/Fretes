...
const sIco='<svg viewBox="0 0 20 20" fill="currentColor" width="15" height="15"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"/></svg>';
...
function toggleTheme(){
  ...
  const m=document.getElementById('meta-theme');if(m)m.content=n==='dark'?'#111114':'#f4f2ed';
  updateThemeLabel();
}
function updateThemeLabel(){const e=document.getElementById('theme-label');if(e)e.textContent=document.documentElement.getAttribute('data-theme')==='light'?'Tema escuro':'Tema claro'}
updateThemeLabel();
...
setInterval(()=>{fetch('/api/sync/').then(r=>r.json()).then(d=>{if(d&&d.status==='ok'){document.getElementById('last-update').textContent=d.atualizado||'';if(P==='dashboard')load()}}).catch(()=>{})},SYNC_INTERVAL);

function nav(p){
  P=p;closeSidebar();
  document.querySelectorAll('.nav-item').forEach(n=>n.classList.toggle('active',n.dataset.page===p));
  document.querySelectorAll('.bnav-item').forEach(n=>n.classList.toggle('active',n.dataset.page===p));
...
function setGrupo(g){G=g;document.querySelectorAll('.grupo-tab').forEach(t=>t.classList.toggle('active',t.dataset.grupo===g));load()}
function load(){document.getElementById('content').innerHTML='<div class="loading"><div class="spinner"></div>Carregando...</div>';({dashboard:loadDash,motoristas:loadMot,colheita:loadColheita,romaneios:loadRom,abastecimentos:loadAbast,adiantamentos:loadAdiant})[P]()}
...
<div class="g21">
  ...
  <div class="card"><div class="card-h"><div class="card-t"><div class="dot red"></div>Devedores</div></div>
    ${d.length===0?'<div style="text-align:center;color:var(--txm);padding:24px;font-size:12px">Nenhum</div>':
    `<div class="scroll-t">${d.map((x,i)=>`<div style="padding:10px;border-bottom:1px solid var(--bd);${i===0?'background:var(--red-bg)':''};cursor:pointer" onclick="openMot(${x.id},'${esc(x.motorista)}','${x.placa}')"><div style="display:flex;justify-content:space-between;align-items:center;gap:6px"><div style="min-width:0"><div style="font-weight:600;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${x.motorista}</div><div class="mono" style="font-size:10px;color:var(--txm);margin-top:1px">${x.placa}</div></div><div class="mono v-neg" style="font-size:12px;font-weight:600;flex-shrink:0">- ${fmt(Math.abs(x.saldo))}</div></div>${x.adiantamentos>0?`<div style="font-size:10px;color:var(--amb);margin-top:3px">Adiant: ${fmt(x.adiantamentos)}</div>`:''}</div>`).join('')}</div>`}
  </div>
</div>
<div class="card"><div class="card-h"><div class="card-t"><div class="dot pur"></div>Tabela Completa</div></div>
  <div class="search-bar">${sIco}<input placeholder="Buscar..." oninput="filt(this,'td')"></div>
  <div class="scroll-t" style="max-height:460px"><table id="td"><thead><tr><th>Placa</th><th>Motorista</th><th class="tr">Rom.</th><th class="tr">Abast.</th><th class="tr">Adiant.</th><th class="tr">Saldo</th><th class="tc">Sit.</th></tr></thead>
